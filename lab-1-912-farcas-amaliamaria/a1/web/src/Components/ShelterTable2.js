...
export default function ShelterTable2({ filterValue }) {
    ...
    React.useEffect(() => {
        let link = "http://localhost:8080/api/shelter";
        if (filterValue !== "") {
            link = `http://localhost:8080/api/shelter/greaterCapacity?capacity=${filterValue}`;
        }
        fetch(link)
